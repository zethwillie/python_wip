...
class XferAnimForm(MelVSingleStretchLayout):
	...
	def setClip( self, clip, mapping=None ):
		...
		self.UI_check1( e=True, l="additive key values", vis=False )
		self.UI_check2( e=True, l="match rotate order", vis=False )
		self.UI_check3( e=True, l="import as world space", vis=True )
	...
	def on_xfer( self, *a ):
		...
		instance = self.UI_check1.getValue()
		traceKeys = self.UI_keysOnly.getValue()
		matchRo = self.UI_check2.getValue()
		startTime = self.UI_start.getValue()
		endTime = self.UI_end.getValue()
		world = processPostCmds = self.UI_check3.getValue()  #this is also "process trace cmds"
		...
		if self._clip is not None:
			mapping = self.UI_mapping.getMapping()
			assert isinstance( self._clip, animClip.AnimClip )
			self._clip.setMapping( mapping ).apply( mapping.tgts, worldSpace=world )
		elif isDupe:
			...
			clip.apply( mapping, applySettings, world )
		elif isTraced:
