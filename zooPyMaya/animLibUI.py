...
class ClipWidget(MelIconRadioButton):
	...
	def buildMenu( self, parent, *args ):
		...
		cmd.menuItem( l=self.clipPreset.name(), boldFont=True )
	...
	def onSlam( self, *a ):
		self.sendEvent( 'clipSlammed', self.clipPreset )
	def onSelectClipItems( self, *a ):
		self.sendEvent( 'clipSelectItems', self.clipPreset )
	def onMapping( self, *a ):
		self.sendEvent( 'clipMapped', self.clipPreset )
	def delete( self ):
		self.clipPreset.delete()

		super( self, ClipWidget ).delete()
...
class ClipsLayout(MelShelfLayout):
	...
	def getClips( self ):
		...
		for library in self._libraries:
			...
			clips[library] = allClips[presets.LOCAL] + allClips[presets.GLOBAL]

		return clips
	def populate( self, showAnim=True, showPose=True ):
		self.clear()
		...
		clipTypesToShow = []
		if showAnim: clipTypesToShow.append( animLib.AnimClipPreset )
		if showPose: clipTypesToShow.append( animLib.PoseClipPreset )
	...
	def setFilter( self, filterStr, showAnim=True, showPose=True ):
		self._filterStr = filterStr
		self.populate( showAnim, showPose )
	def buildMenu( self, parent, *args ):
		cmd.setParent( parent, m=True )
		cmd.menu( parent, e=True, dai=True )
...
class ClipApplyLayout(MelVSingleStretchLayout):
	def __init__( self, parent ):
		...
		self.UI_opt_worldSpace = MelCheckBox( self, l='load clip in world space', v=False )
		self.UI_opt_attrSelection = MelCheckBox( self, l='use attribute selection', v=False, en=False )
	...
	def update( self ):
		uisToEnable = self.UI_opt_currentTime, self.UI_opt_worldSpace
	...
	def getApplyKwargs( self ):
		...
		kwargs['applySettings'] = self.UI_opt_currentTime.getValue()
		kwargs['worldSpace'] = self.UI_opt_worldSpace.getValue()
		#kwargs['attrs'] = self.UI_opt_attrSelection.getValue()
	...
	def onDrag( self, value ):
		...
		blendedClip = self._preClip.blend( self._mappedClip, value )
...
class AnimLibLayout(MelHSingleStretchLayout):
	...
	def on_filterClips( self, *a ):
		...
		self.UI_clips.setFilter( self.UI_filterClips.getValue(), showAnim, showPose )
