		...
		for node in nodes:
			if node in self._nodeDict:
				...
				for attrName, keyData in dataDict.iteritems():
					...
					try:
						keyData.apply( attrPath, applySettings.sourceRange, timeOffset )
...
class AnimClip(BaseClip):
	...
	def apply( self, nodes=None, applySettings=None, worldSpace=False ):
		self._channelClip.apply( nodes, applySettings )
		if worldSpace and self._transformClip:
			self._transformClip.apply( nodes, applySettings )
	...
	def blend( self, other, amount ):
		...
		for node, nodeAttrDict in self._nodeAttrDict.iteritems():
			...
			for attr, value in nodeAttrDict.iteritems():
				...
				blendedValue = newAttrDict[ attr ] = (value * (1-amount)) + (otherValue * amount)
